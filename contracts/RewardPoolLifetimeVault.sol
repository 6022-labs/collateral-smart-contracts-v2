...
import {IRewardPoolLifetimeVault} from "./interfaces/IRewardPoolLifetimeVault/IRewardPoolLifetimeVault.sol";
 ...
 */
contract RewardPoolLifetimeVault is Ownable, BaseVault, IRewardPoolLifetimeVault {
    // ----------------- VARIABLES ----------------- //
    ...
    ) Ownable(_owner) BaseVault(_rewardPool, _wantedAmount) {
        isDeposited = false;
        isWithdrawn = false;
    ...
    modifier onlyWhenLastVaultInPool() {
        ...
        if (IRewardPoolStates(rewardPool).countRewardableVaults() != 1)
        ...
        onlyWhenLastVaultInPool
    {
        ...
        IRewardPoolVaultActions(rewardPool).harvestRewards(_msgSender());
