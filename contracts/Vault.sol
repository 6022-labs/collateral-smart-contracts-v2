...
import {IVault} from "./interfaces/IVault/IVault.sol";
import {IVaultDescriptor} from "./interfaces/IVaultDescriptor.sol";
import {IControllerStates} from "./interfaces/IController/IControllerStates.sol";
import {IRewardPoolStates} from "./interfaces/IRewardPool/IRewardPoolStates.sol";
import {IRewardPoolVaultActions} from "./interfaces/IRewardPool/IRewardPoolVaultActions.sol";
 ...
 */
contract Vault is ERC721, BaseVault, ReentrancyGuard, IVault {
    // ----------------- CONST ----------------- //
    ...
    ) ERC721(_name, "6022") BaseVault(_rewardPoolAddress, _wantedAmount) {
        creator = _creator;
        ...
        nonReentrant
    {
        ...
        if (requiredNFTs == WITHDRAW_NFTS_LATE) {
            IRewardPoolVaultActions(rewardPool).harvestRewards(msg.sender);
        } else {
            IRewardPoolVaultActions(rewardPool).reinvestRewards();
        }
    }
    ...
    function vaultOverview() external view returns (VaultOverview memory) {
        ...
        ITokenOperation wantedToken = ITokenOperation(wantedTokenAddress);
        IRewardPoolStates rewardPoolStates = IRewardPoolStates(
            rewardPool
        );
        ...
        returns (string memory)
    {
        ...
        address controllerAddress = IRewardPoolStates(rewardPool)
            .controller();
        address vaultDescriptorAddress = IControllerStates(controllerAddress)
            .vaultDescriptor();

        return IVaultDescriptor(vaultDescriptorAddress).buildTokenURI(
            address(this),
            tokenId
        );
    }
