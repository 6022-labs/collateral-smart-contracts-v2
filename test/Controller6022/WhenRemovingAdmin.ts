...
describe("When removing admin from controller 6022", function () {
  ...
  describe("Given caller is an admin", async function () {
    beforeEach(async function () {
      await _controller6022.addAdmin(await _otherAccount.getAddress());
    });

    it("Should emit 'AdminRemoved' event", async function () {
      await expect(
        _controller6022.removeAdmin(await _otherAccount.getAddress())
      ).to.emit(_controller6022, "AdminRemoved");
    });
  });
});
