...
describe("When updating vault descriptor to controller 6022", function () {
  ...
  beforeEach(async function () {
    const { controller6022, otherAccount } = await loadFixture(
      deployController
    );
  ...
  describe("Given caller is an admin", async function () {
    it("Should emit 'VaultDescriptorUpdated' event", async function () {
      await expect(
        _controller6022.updateVaultDescriptor(await _otherAccount.getAddress())
      ).to.emit(_controller6022, "VaultDescriptorUpdated");
    });
  });
});
